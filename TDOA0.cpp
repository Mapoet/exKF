...
int main(int argc, char **argv)
{
    ...
    double lambda=argc>=2?atof(argv[1]):0.1,beta=5e-9;
    EXKF::Array  x0={0.0,0.0,0.0},p={0.0},theory={0.0,0.0,0.0},measure;
    IArray obsidx,iobs;
    ...
    for (auto i=0;i<10000;i++){
        p[0]=0.1*i;
        kf.predict(p,Q);
        theory[0]=0.1*i*sin(p[0]/300*3.14159265)*(1+randn(gen)*0.01)+0.1*i;
        theory[1]=0.2*i*sin(p[0]/500*3.14159265)*(1+randn(gen)*0.01)+0.2*i;
        ...
        //argsin:Xms,Yms,dTms
        kf.update_linear(measure,getR(k,beta),[iobs,measure,TS](const EXKF::DiffArray &argsin, const EXKF::Array &parameter) {
